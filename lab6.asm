...
;======================================================
def_max equ 10              ; початкова кількість "одночасно"
                            ; виконуваних  задач
...
_DATA SEGMENT WORD PUBLIC 'DATA' use16
    ...
    fon equ def_max         ; ознака фонової задачі;
    ...
    nprg dw 0               ; номер активної задачі (від 0 до def_max-1)
    ...
    max_prg dw def_max      ; лічильник поточної кількісті активних задач
_DATA ENDS
...
; перервана задача фонова
    ...
    mov ax, max_prg
    mov nprg, ax
    dec nprg                ; забезпечити перегляд задач з 0-вої
    mov cx, def_max         ; max_prg - max кількість задач
    jmp disp015
...
; перервана задача не фонова
    ...
    ; якщо видалили перервану задачу
    cmp init[si], absent
    jne disp013

    ; видаляемо напис на екрані
    cld
    mov ah, 10b
    mov al, 0
    mov cx, 25                  ; 22 розряда у лічильника + 3 символи назви лічильника
    mov di, screen_addr[esi*2]  ; координата лічильника
    rep stosw

    ; запускаємо фонову задачі замість видаленої
    mov sp, fonsp
    mov nprg, fon
    pop ds
    popad
...
;----------------------------------------------------------------------------
    ...
    f8_key equ 42h          ; скан-код клавіші F8
...
ui9020:
    ...
    push max_prg                ; виведення на екран поточної
    push 2760                   ; кількості активних задач
    call show
;*****************************************************************
ui9040:
